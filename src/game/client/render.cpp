...
void CRenderTools::RenderHammerHitbox(vec2 TeePos, vec2 Direction) const
{
	...
	Graphics()->QuadsBegin();
	Graphics()->SetColor(color_cast<ColorRGBA>(ColorHSLA(g_Config.m_ClShowHammerHitboxColor, true)));
...
void CRenderTools::RenderTeeHitbox(vec2 Pos) const
{
	...
	Graphics()->TextureClear();
	Graphics()->QuadsBegin();
	Graphics()->SetColor(color_cast<ColorRGBA>(ColorHSLA(g_Config.m_ClShowTeeHitboxOuterColor, true)));
	Graphics()->DrawCircle(Pos.x + Offset, Pos.y + Offset, 28.f + Offset, 100);
	Graphics()->SetColor(color_cast<ColorRGBA>(ColorHSLA(g_Config.m_ClShowTeeHitboxInnerColor, true)));
